...
def get_refresh_token():
    return jwt.encode({
        'exp': datetime.now() + timedelta(days=128), 'data': get_random(10)},
        settings.SECRET_KEY,
        algorithm="HS256"
        )
...
class RefreshView(APIView):
    ...
    def post(self, request):
        ...
        #try to validate if it's stilll active 
# a = Authentication() if it wasn't a static method
...
class GetSecuredInfo(APIView):
    ...
    def get(self, request):
        ...
        return Response({'data': 'This a secured information'})
